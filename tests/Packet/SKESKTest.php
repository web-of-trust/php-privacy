...
class SKESKTest extends OpenPGPTestCase
{
    ...
    public function testEncryptNullSessionKey()
    {
        ...
        $this->assertSame(
            SymmetricKeyEncryptedSessionKey::VERSION_4,
            $skesk->getVersion()
        );
        ...
        $packets = PacketList::decode(
            (new PacketList([$skesk, $seipd]))->encode()
        );
        $skesk = $packets->offsetGet(0)->decrypt(self::PASSPHRASE);
        $seipd = $packets
            ->offsetGet(1)
            ->decryptWithSessionKey($skesk->getSessionKey());
    ...
    public function testAeadEaxDecryption()
    {
        ...
        $skesk = SymmetricKeyEncryptedSessionKey::fromBytes(
            base64_decode($skeskData)
    ...
    public function testEncryptedUsingArgon2()
    {
        ...
        $sessionKey = $skesk->getSessionKey();
        $this->assertSame(
            "01fe16bbacfd1e7b78ef3b865187374f",
            bin2hex($sessionKey->getEncryptionKey())
        );
        ...
        $sessionKey = $skesk->getSessionKey();
        $this->assertSame(
            "27006dae68e509022ce45a14e569e91001c2955af8dfe194",
            bin2hex($sessionKey->getEncryptionKey())
        );
