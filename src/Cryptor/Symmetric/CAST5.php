...
class CAST5 extends BlockCipher
{
    ...
    protected function setupKey(): void
    {
         ...
         * correct int.
         */
        $x03 = self::intsTo32bits($x, 0x0);
        $x47 = self::intsTo32bits($x, 0x4);
        $x8b = self::intsTo32bits($x, 0x8);
        $xcf = self::intsTo32bits($x, 0xc);
        $z03 = $x03 ^
            self::$sBox5[$x[0xd]] ^
            self::$sBox6[$x[0xf]] ^
            self::$sBox7[$x[0xc]] ^
        ...
        $z03 = self::intsTo32bits($z, 0x0);
        $z47 = self::intsTo32bits($z, 0x4);
        $z8b = self::intsTo32bits($z, 0x8);
        $zcf = self::intsTo32bits($z, 0xc);
        $x03 = $z8b ^
            self::$sBox5[$z[0x5]] ^
            self::$sBox6[$z[0x7]] ^
            self::$sBox7[$z[0x4]] ^
    ...
    private static function f1(int $d, int $kmi, int $kri): int
    {
        ...
        return (self::$sBox1[($i >> 24) & Bitwise::MASK_8BITS] ^
            self::$sBox2[($i >> 16) & Bitwise::MASK_8BITS]) -
            self::$sBox3[($i >> 8) & Bitwise::MASK_8BITS] +
            self::$sBox4[$i & Bitwise::MASK_8BITS];
    }
    ...
    private function encipher(int $l0, int $r0): array
    {
        ...
        for ($i = 1; $i <= $this->rounds; $i++) {
            ...
            $ri = match ($i) {
                1, 4, 7, 10, 13, 16 => $lp ^
                    self::f1($rp, $this->masking[$i], $this->rotating[$i]),
                2, 5, 8, 11, 14 => $lp ^
                    self::f2($rp, $this->masking[$i], $this->rotating[$i]),
                3, 6, 9, 12, 15 => $lp ^
                    self::f3($rp, $this->masking[$i], $this->rotating[$i]),
                default => $ri,
            };
        }
        return [$ri, $li];
     ...
     * @return array<int>
     */
    private static function bits32ToInts(
        int $input, array $bytes, int $offset,
    ): array {
