...
class Camellia extends BlockCipher
{
    ...
    protected function isValidEngineHelper($engine): bool
    {
        switch ($engine) {
            case self::ENGINE_OPENSSL:
                ...
                $this->cipher_name_openssl_ecb =
                    "camellia-" . $this->getKeyLength() . "-ecb";
    ...
    protected function encryptBlock($input): string
    {
        ...
        if ($this->keyIs128) {
            ...
            [$ka, $this->subkey] = self::roldqo32(
                34,         $ka,         0,         $this->subkey,         28,     );
    ...
    protected function decryptBlock($input): string
    {
        ...
        if ($this->keyIs128) {
            ...
            [$k, $this->subkey] = self::decroldqo32(
                34,         $k,         4,         $this->subkey,         8,     );
            /* KA dependant keys */
            [$ka, $this->subkey] = self::decroldq(
                15,         $ka,         0,         $this->subkey,         36,     );
            [$ka, $this->subkey] = self::decroldq(
                30,         $ka,         0,         $this->subkey,         24,     );
            /* 32bit rotation */
            $this->ke[2] = $ka[1];
            $this->ke[3] = $ka[2];
            $this->ke[0] = $ka[3];
            ...
            [$kb, $this->subkey] = self::decroldq(
                30,         $kb,         0,         $this->subkey,         32,     );
            [$kb, $this->subkey] = self::decroldq(
                30,         $kb,         0,         $this->subkey,         16,     );
    ...
    protected function setupKey(): void
    {
        ...
        $this->keyIs128 = match (strlen($this->key)) {
            16 => true,     24, 32 => false,     default => throw new \LengthException(
    ...
    private static function roldq(
        ...
        $ko[0 + $ooff] =
            ...
            Bitwise::rightShift32($ki[1 + $ioff], 32 - $rot);
        $ko[1 + $ooff] =
            Bitwise::leftShift32($ki[1 + $ioff], $rot) |
            Bitwise::rightShift32($ki[2 + $ioff], 32 - $rot);
        $ko[2 + $ooff] =
            Bitwise::leftShift32($ki[2 + $ioff], $rot) |
            Bitwise::rightShift32($ki[3 + $ioff], 32 - $rot);
        $ko[3 + $ooff] =
            Bitwise::leftShift32($ki[3 + $ioff], $rot) |
            Bitwise::rightShift32($ki[0 + $ioff], 32 - $rot);
        $ki[0 + $ioff] = $ko[0 + $ooff];
        $ki[1 + $ioff] = $ko[1 + $ooff];
        $ki[2 + $ioff] = $ko[2 + $ooff];
        $ki[3 + $ioff] = $ko[3 + $ooff];
    ...
    private static function roldqo32(
        ...
        $ko[0 + $ooff] =
            ...
            Bitwise::rightShift32($ki[2 + $ioff], 64 - $rot);
        $ko[1 + $ooff] =
            Bitwise::leftShift32($ki[2 + $ioff], $rot - 32) |
            Bitwise::rightShift32($ki[3 + $ioff], 64 - $rot);
        $ko[2 + $ooff] =
            Bitwise::leftShift32($ki[3 + $ioff], $rot - 32) |
            Bitwise::rightShift32($ki[0 + $ioff], 64 - $rot);
        $ko[3 + $ooff] =
            Bitwise::leftShift32($ki[0 + $ioff], $rot - 32) |
            Bitwise::rightShift32($ki[1 + $ioff], 64 - $rot);
        $ki[0 + $ioff] = $ko[0 + $ooff];
        $ki[1 + $ioff] = $ko[1 + $ooff];
        $ki[2 + $ioff] = $ko[2 + $ooff];
        $ki[3 + $ioff] = $ko[3 + $ooff];
